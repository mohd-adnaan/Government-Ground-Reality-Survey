...
const saveDataClicked = () => {
  if ((capturedImg.image.length == 0) || (searchresult.selclass=='') ) 
  {
...
const saveData=async(type)=>{
 ...
 // let timestamp=d.getDate()+"-"+(d.getMonth()+1)+"-"+d.getFullYear()+" "+d.getHours()+":"+d.getMinutes()+":"+d.getSeconds();
  ...
  db.transaction((tx) => {
    tx.executeSql(
      'INSERT INTO geotagTbl (timestamp,addedby,selstate,seldist,l1class,l2class,l3class,regdisc,otherclass,placename,num_img,photob640,pic1time,latitude1,longitude1,accuracy1,photob641,pic2time,latitude2,longitude2,accuracy2,obssts) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',         
      [timestamp,addedby,data.selState,data.selDist,data.selLevel1,data.selLevel2,data.selLevel3,data.selLevel4,data.rem,data.placename,numimg,photob640,
      ...
      (tx,results) => {
        ...
        if (type=='Sent')  {
         ...
         // Alert.alert("Success","Data successfully saved in History");
          Alert.alert("Success","Data successfully saved in History",
      [{ text: 'OK', onPress: () => { navigation.navigate("Main")}}],{ cancelable: false }
      );
          resetForm();
...
const sendToServer =async(id)=>
{
  ...
  useEffect(() => {
   ...
   // getStatelist();
    db.transaction((tx) => {
          tx.executeSql(
            "create table if not exists geotagTbl ( id INTEGER PRIMARY KEY AUTOINCREMENT,timestamp text,addedby text, selstate text,seldist text,l1class text, l2class text, l3class text, regdisc text, otherclass text, placename text, num_img int, photob640 text,pic1time text, latitude1 numeric,longitude1 numeric,accuracy1 numeric,photob641 text,  pic2time text, latitude2 numeric,longitude2 numeric,accuracy2 numeric,obssts text);",     
            [], (tx) => {
              console.log('----created---');
         },(t,error)=>{
          console.log(error);
    ...
    return ((data.isLoading) ? (
      ...
      <ScrollView style={styles.scrollContainer} keyboardShouldPersistTaps='always'>
          ...
          <Text style={[styles.getStartedText, {marginTop:10}]}>Place Name</Text>
              <TextInput
                ...
                onChangeText={placename => {
                  setData({
                      ...data,
                      placename: placename.replace(/[`~!@#$%^&*_|+\=?;:'",.<>\{\}\[\]\\\/]/gi, '') })
                }} />

          <Text style={styles.headerText}>LU/LC Class</Text>
        ...
        <RNPicker
            ...
            />
              ...
              <View style={styles.cameracontainer}>
                  ...
                  if (gpsloc.accvalue>300)             
                      alert('Accuracy of GPS is more than 30 m. Please wait sometime for accuracy to improve. Ensure you are under open sky!');
                  else{
                    Alert.alert("Capture Photo","Capturing photo with accuracy "+gpsloc.accvalue+ ' m',
                    [{ text: 'OK', onPress: () => {                      
                      pickImage();
                    }}])
